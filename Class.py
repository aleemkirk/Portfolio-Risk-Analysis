...
class PortfolioMetrics:
    ...
    def __init__(self, securities, weights=None, market=None, start_date=None, end_date=None, trading_days=None) -> None:
        ...
        # assume equal investment if weights are not specified
        self.weights = weights or [
            1/self.num_securities for _ in range(self.num_securities)]
        ...
        if len(self.securities) != len(self.weights):
            raise Exception(
                'Investment weights and securities are not the same length')
    ...
    def getData(self) -> pd.DataFrame:
        ...
        try:
            for ticker in self.portfolio:
                ...
                self.data = pd.merge(
                    self.data, df[['date', 'close']], on='date', how='outer', suffixes=(ticker, ticker))
        except:
    ...
    def dailyROR(self) -> pd.DataFrame:
        ...
        if self.data.empty:
            raise Exception(
                "Securities data does not exist. Try calling getData() first.")

        self.daily_ROR[self.portfolio] = (
            self.data[self.portfolio].diff()/self.data[self.portfolio].shift(1))*100
    ...
    def meanDailyROR(self) -> pd.Series:
        ...
        if self.daily_ROR.empty:
            raise Exception(
                "Return information does not exist. Try calling meanDailyROR() first.")
    ...
    def elasticity(self) -> pd.Series:
        ...
        if self.cov_daily_ROR.empty:
            raise Exception(
                "Covariance information does not exist. Try calling covDailyROR() first.")

        self.beta = (
            self.cov_daily_ROR.iloc[-1, :self.num_securities])/self.daily_ROR[self.market].var(axis=0)
    ...
    def annReturn(self) -> np.ndarray:
        ...
        if self.mean_daily_ROR.empty:
            raise Exception(
                "Mean daily return does not exist. Try calling meanDailyROR() first.")

        self.annual_return = self.trading_days * \
            np.matmul(
                self.weights, self.mean_daily_ROR[:self.num_securities].to_list()).sum()
    ...
    def annMarketReturn(self) -> np.ndarray:
        ...
        self.annual_market_return = self.trading_days * \
            (self.mean_daily_ROR[self.market])
    ...
    def annRisk(self) -> np.ndarray:
        ...
        if self.cov_daily_ROR.empty:
            raise Exception(
                "Covariance information does not exist. Try calling covDailyROR() first.")
        self.annual_risk = np.sqrt(self.trading_days*np.matmul(np.matmul(
            self.weights, self.cov_daily_ROR.iloc[:self.num_securities, :self.num_securities].to_numpy()), np.transpose(self.weights)))
    ...
    def divIndex(self):
        ...
        self.div_index = np.matmul(self.daily_ROR[self.securities].std().to_numpy(), np.transpose(self.weights))/np.sqrt(np.matmul(
            self.weights, np.matmul(self.cov_daily_ROR.iloc[:self.num_securities, :self.num_securities].to_numpy(), np.transpose(self.weights))))
...
class PortfolioDiversifier(PortfolioMetrics):
    ...
    def diversify(self):
        ...
        features = np.concatenate([self.mean_daily_ROR[self.securities].to_numpy().reshape(len(
            self.weights), 1), self.cov_daily_ROR.iloc[:self.num_securities, :self.num_securities].to_numpy()], axis=1)
        cluster = KMeans(algorithm='lloyd', max_iter=100,
